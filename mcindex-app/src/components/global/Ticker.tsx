...
const fetchCountryPriceDataForTicker = async (
  year: number = 2025
): Promise<FetchedCountryProductInfo[]> => {
...
type StockTickerProps = React.HTMLAttributes<HTMLDivElement>;
...
export function StockTicker({ className, ...props }: StockTickerProps) {
  ...
  const {
    ...
    isLoading: isLoadingCountryPrices,
    isError: isErrorCountryPrices,
  ...
  const tickerItems = useMemo((): TickerItemData[] => {
    ...
    return countryPriceData.map((country: FetchedCountryProductInfo) => {
      ...
      const productDetails = productsData.products.find(
        (p) => p.id === country.selectedProductId
      );
  ...
  if (isErrorProducts || isErrorCountryPrices) {
    return (
      <div
        className={cn(
          "h-10 flex items-center justify-center text-sm text-red-600",
          className
        )}
        {...props}
      >
  ...
  return (
    <div className="w-full bg-muted/50 border-y overflow-hidden font-mono">
      <div
        className={cn(
          `container mx-auto flex items-center justify-between py-1 gap-6`,
          className
        )}
      >
        ...
        >
          ...
          >
            {duplicatedItems.map((item, index) => {
              const priceDiff =
                (item.currentPrice ?? 0) - (item.previousPrice ?? 0);
              let TrendIcon = Minus;
